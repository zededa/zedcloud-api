...
func (m *ConfigEdgeDevConfig) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateConfigTimestamp(formats); err != nil {
		res = append(res, err)
	}
...
func (m *ConfigEdgeDevConfig) validateConfigTimestamp(formats strfmt.Registry) error {
	if swag.IsZero(m.ConfigTimestamp) { // not required
		return nil
	}

	if err := validate.FormatOf("configTimestamp", "body", "date-time", m.ConfigTimestamp.String(), formats); err != nil {
		return err
	}
