...
func (m *ConfigPhysicalIO) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateVflist(formats); err != nil {
		res = append(res, err)
	}
...
func (m *ConfigPhysicalIO) validateVflist(formats strfmt.Registry) error {
	if swag.IsZero(m.Vflist) { // not required
		return nil
	}

	if m.Vflist != nil {
		if err := m.Vflist.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("vflist")
			} else if ce, ok := err.(*errors.CompositeError); ok {
				return ce.ValidateName("vflist")
			}
			return err
		}
	}
...
func (m *ConfigPhysicalIO) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	...
	if err := m.contextValidateVflist(ctx, formats); err != nil {
		res = append(res, err)
	}
